...
# --- Universal Safe Attribute / Key Extractors ---
...
def show_auth_page() -> None:
            ...
            if not clean_u or not clean_p:
                ...
                if user_data:
                    if not get_int(user_data, "is_active", 1):
                        st.error("This account has been deactivated. Contact your station owner or admin.")
                    else:
    ...
    with tab_otp_login:
        ...
        st.caption("Re-enter your corrected details below if you made an error previously.")
        ...
        if st.button("Send Login OTP", type="primary", use_container_width=True):
            ...
            if not clean_user or not clean_email:
                ...
                if not user:
                    ...
                    if send_email_notification(clean_email, "Your Login OTP", f"Your OTP code is: <strong>{code}</strong>"):
    ...
    with tab_join:
        ...
        st.caption("Apply as an Admin, Cashier, or Delivery Driver. The Business Owner will be notified via email to approve your request.")
        ...
        appl_email = st.text_input("Your Email Address", key="join_email")
        ...
        if st.button("Submit Join Application", type="primary", use_container_width=True):
            ...
            if not clean_appl_e or "@" not in clean_appl_e:
                ...
                if not owner:
                    st.error("Business Owner details do not match. Verify owner username, email, and station name.")
                else:
                    ...
                    if not is_valid:
                        ...
                        if sent:
                            ...
                            st.warning("Application created! The owner can approve your request directly from their workforce dashboard.")
        ...
        with col_right:
            ...
            if st.session_state.cart:
                cart_df = pd.DataFrame(
                    [
                        {
                            "Product ID": get_int(item, "product_id"),
                            "Qty": get_int(item, "quantity"),
                            "Price": f"₱{get_float(item, 'unit_price'):.2f}",
                            "Subtotal": f"₱{get_float(item, 'unit_price') * get_int(item, 'quantity'):.2f}",
                        }
                ...
                total_amount = sum(
                    get_float(item, "unit_price") * get_int(item, "quantity") for item in st.session_state.cart
                )
    ...
    # ==========================================
        ...
        with tab_staff:
            ...
            for member in staff_list:
                ...
                if m_id == user_id:
                    continue  # Skip managing self in workforce grid
        ...
        with tab_reqs:
            ...
            if not pending:
                ...
                for req in pending:
                    ...
                    with st.container(border=True):
                        st.markdown(f"**{get_str(req, 'applicant_username')}** applying for `{get_str(req, 'requested_role').upper()}`")
